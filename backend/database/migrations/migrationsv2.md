...
Automergeの変更バイナリ（change）を溜めるだけの、追記専用（append-only）の配送用ログ。
サーバー（PHP）にはAutomergeの実装が無いため、change_blobの中身は一切解釈しない。
マージ処理は必ずクライアント側（Automerge JS / crdtStore.ts）が担当する。

| カラム名 | 型 | 属性 | 説明 |
|---|---|---|---|
| id | bigint | PRIMARY KEY, AUTO INCREMENT | pullカーソルとして使う連番ID |
| user_id | string | FK → users.user_id | 変更の所有者ユーザーID |
| device_id | string |  | 変更を発行した端末ID（ログ・デバッグ用。マージ判定には使わない） |
| change_blob | text |  | Automergeの変更バイナリをbase64化したもの |
| created_at | timestamp | useCurrent() | サーバーに届いた日時（updated_atは無し） |
