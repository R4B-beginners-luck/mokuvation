...
import { HELP_FAQ_GROUPS } from './helpFaqData';

// ヘルプ機能の中身（FAQ アコーディオン）
export function HelpContent() {
  return (
    <div className="legal-modal-content help-faq">
      {HELP_FAQ_GROUPS.map((group) => (
        <section key={group.id} className="help-faq__group">
          <h3 className="help-faq__group-title">{group.title}</h3>
          <div className="help-faq__list">
            {group.items.map((item) => (
              <FaqAccordionItem key={item.id} question={item.question}>
                {item.answer}
              </FaqAccordionItem>
            ))}
          </div>
...
export function TermsContent() {
  return (
    <div className="legal-modal-content" style={{ padding: '10px 0', lineHeight: '1.6' }}>
      ...
      <p style={{ marginTop: '16px', fontSize: '12px', color: 'var(--text-muted)' }}>制定日・改定日　2026年7月16日</p>
    </div>
  );
}
...
export function PrivacyContent() {
  return (
    <div className="legal-modal-content" style={{ padding: '10px 0', lineHeight: '1.6' }}>
      ...
      <ul style={{ paddingLeft: '20px', margin: '8px 0' }}>
        <li>Vercel（フロントエンドの公開）</li>
        <li>Railway（バックエンドの公開）</li>
        <li>データベースサービス</li>
      </ul>
